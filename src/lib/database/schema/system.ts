...
export const dataPointTemplates = sqliteTable(
...
export const extractions = sqliteTable(
  'extractions',
  {
    ...
    templateId: text('template_id')
      .notNull()
      .references(() => extractionTemplates.id, { onDelete: 'cascade' }),
...
export const dataPoints = sqliteTable(
  'data_points',
  {
    ...
    templateId: text('template_id')
      .notNull()
      .references(() => dataPointTemplates.id, { onDelete: 'cascade' }),
...
export const summarizations = sqliteTable(
  'summarizations',
  {
    id: text('id')
      .primaryKey()
    ...
    fileId: integer('file_id')
      .notNull()
      .references(() => audioFiles.id, { onDelete: 'cascade' }),
    templateId: text('template_id').references(() => summarizationPrompts.id, {
      onDelete: 'set null',
    }),
    model: text('model').notNull(),
...
export const summarizationPrompts = sqliteTable(
...
export const extractionDefinitions = sqliteTable(
  'extraction_definitions',
  {
    ...
    category: text('category', { enum: ['extraction', 'datapoint'] }).default(
      'extraction',
    ),
    isActive: integer('is_active', { mode: 'boolean' }).default(true),
...
export const extractionResults = sqliteTable(
  'extraction_results',
  {
    ...
    definitionId: text('definition_id')
      .notNull()
      .references(() => extractionDefinitions.id, { onDelete: 'cascade' }),
...
export const aiProcessingSessions = sqliteTable(
  'ai_processing_sessions',
  {
    ...
    fileId: integer('file_id')
      .notNull()
      .references(() => audioFiles.id, { onDelete: 'cascade' }),
    summarizationPromptId: text('summarization_prompt_id').references(
      () => summarizationPrompts.id,
      { onDelete: 'set null' },
    ),
    extractionDefinitionIds: text('extraction_definition_ids', {
      mode: 'json',
    }).$type<string[]>(), // IDs of active extractions
...
export const realTimeSessions = sqliteTable(
  'real_time_sessions',
  {
    id: text('id')
      .primaryKey()
      .$defaultFn(() => createId()),
    fileId: integer('file_id').references(() => audioFiles.id, {
      onDelete: 'cascade',
    }), // null while recording, set when saved
    startedAt: integer('started_at', { mode: 'timestamp' })
      .notNull()
      .default(sql`CURRENT_TIMESTAMP`),
    endedAt: integer('ended_at', { mode: 'timestamp' }),
    totalChunks: integer('total_chunks').default(0),
    status: text('status', { enum: ['active', 'completed', 'failed'] }).default(
      'active',
    ),
...
export const realTimeThoughts = sqliteTable(
  'real_time_thoughts',
  {
    id: text('id')
      .primaryKey()
