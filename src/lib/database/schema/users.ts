...
export const systemSettings = sqliteTable('system_settings', {
  ...
  isInitialized: integer('is_initialized', { mode: 'boolean' })
    .default(false)
    .notNull(),
  ...
  // Whisper settings
  whisperModelSizes: text('whisper_model_sizes', { mode: 'json' })
    .$type<string[]>()
    .default(['tiny', 'base', 'small', 'medium', 'large']),
  whisperQuantization: text('whisper_quantization').default('none'),

  // Storage settings
  obsidianEnabled: integer('obsidian_enabled', { mode: 'boolean' }).default(
    false,
  ),
  ...
  // AI Extract settings
  aiExtractEnabled: integer('ai_extract_enabled', { mode: 'boolean' }).default(
    false,
  ),
  ...
  psychEvalEnabled: integer('psych_eval_enabled', { mode: 'boolean' }).default(
    false,
  ),
  psychEvalAutoRun: integer('psych_eval_auto_run', { mode: 'boolean' }).default(
    false,
  ),
  ...
  realTimeEnabled: integer('real_time_enabled', { mode: 'boolean' }).default(
    false,
  ),
  realTimeChunkInterval: integer('real_time_chunk_interval').default(120), // Interval in seconds (default 2 minutes)
  realTimeAiInstruction: text('real_time_ai_instruction').default(
