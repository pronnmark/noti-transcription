...
export const telegramNotificationPreferences = sqliteTable(
  'telegram_notification_preferences',
  {
    ...
    // Notification types
    transcriptionComplete: integer('transcription_complete', {
      mode: 'boolean',
    }).default(true),
    transcriptionFailed: integer('transcription_failed', {
      mode: 'boolean',
    }).default(true),
...
export const telegramUsers = sqliteTable(
  'telegram_users',
  {
    ...
    monthlyTranscriptionLimit: integer('monthly_transcription_limit').default(
      100,
    ),
...
export const telegramScheduledNotifications = sqliteTable(
  'telegram_scheduled_notifications',
  {
    id: integer('id').primaryKey({ autoIncrement: true }),
    chatId: text('chat_id').notNull(),
    type: text('type', {
      enum: ['daily_digest', 'weekly_report', 'custom'],
    }).notNull(),
  ...
  table => ({
    ...
    nextRunIdx: index('telegram_scheduled_notif_next_run_idx').on(
      table.nextRunAt,
    ),
...
export type TelegramNotificationPreferences =
  typeof telegramNotificationPreferences.$inferSelect;
export type NewTelegramNotificationPreferences =
  typeof telegramNotificationPreferences.$inferInsert;
export type TelegramUser = typeof telegramUsers.$inferSelect;
export type NewTelegramUser = typeof telegramUsers.$inferInsert;
export type TelegramScheduledNotification =
  typeof telegramScheduledNotifications.$inferSelect;
export type NewTelegramScheduledNotification =
  typeof telegramScheduledNotifications.$inferInsert;
