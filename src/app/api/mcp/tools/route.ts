...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      sessionToken !== process.env.AUTH_PASSWORD &&
      sessionToken !== 'ddash'
    ) {
    ...
    switch (toolName) {
      ...
      case 'mcp__telegram-mcp__send_message_to_user':
        ...
        try {
          ...
          const normalizedUsername = args.username
            .toLowerCase()
            .replace('@', '');
          ...
          if (!chatId) {
            ...
            if (!result) {
              ...
              return NextResponse.json({ error: helpMessage }, { status: 400 });
            }
          } else {
      ...
      case 'mcp__telegram-mcp__send_message_to_group':
        ...
        try {
          ...
          const normalizedGroupName = args.group_name
            .toLowerCase()
            .replace('@', '');
