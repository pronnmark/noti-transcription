...
import { AudioRepository, TranscriptionRepository, SummarizationTemplateRepository } from '@/lib/database/repositories';
...
export const POST = withAuthMiddleware(async (
  ...
  try {
    ...
    if (summarizationPromptId) {
      const validTemplate = await summarizationRepo.findById(summarizationPromptId);
      if (!validTemplate) {
        debugLog('api', `❌ Invalid summarization template ID: ${summarizationPromptId}`);
        return NextResponse.json(
          { error: 'Invalid summarization template ID provided', invalidTemplateId: summarizationPromptId },
          { status: 400 }
        );
      }
    }
    ...
    debugLog('api', `🤖 Starting dynamic AI processing for file ${fileIdInt}`);
    debugLog('api', `📝 Summarization prompt: ${summarizationPromptId || 'default'}`);
    debugLog('api', `🔍 Extraction definitions: ${extractionDefinitionIds.join(', ')}`);
    ...
    try {
      ...
      debugLog('api', `📋 Generated system prompt (${systemPrompt.length} chars)`);
      ...
      await getDb().insert(aiProcessingSessions).values({
        id: sessionId,
        fileId: fileIdInt,
      ...
      debugLog('api', `📊 Sample response:`, aiResponse.substring(0, 200) + '...');

      // Update session with AI response
      await getDb().update(aiProcessingSessions)
        .set({
          aiResponse,
          status: 'completed',
      ...
      debugLog('api', `✅ Dynamic AI processing completed for file ${fileIdInt}`);
      debugLog('api', `📊 Extracted ${extractionResults.length} result groups`);
      ...
      try {
        ...
        return NextResponse.json(
          { error: 'Failed to process with AI and fallback failed', details: String(aiError), sessionId },
          { status: 500 }
        );
      }
    }
