...
export async function POST(
  ...
  return withAuthMiddleware(async (req: NextRequest, context) => {
    ...
    try {
      ...
      const summarizationTemplateRepo = RepositoryFactory.summarizationTemplateRepository;

      // Validate template IDs exist in database before processing
      if (templateIds.length > 0) {
        const validTemplates = await summarizationTemplateRepo.findActiveByIds(templateIds);
      ...
      if (!transcriptionJob) {
        return NextResponse.json(
          createErrorResponse('Transcription job not found', 'TRANSCRIPTION_NOT_FOUND', 404),
          { status: 404 }
        );
      }
      ...
      if (!transcriptionJob.transcript) {
        return NextResponse.json(
          createErrorResponse('File not transcribed yet', 'NOT_TRANSCRIBED', 400),
          { status: 400 }
        );
      }

      debugLog('api', `Starting AI processing for file ${fileIdInt}, type: ${processType}`);
      ...
      try {
        switch (processType) {
          ...
          case 'extractions':
            if (templateIds.length === 0) {
              return NextResponse.json(
                createErrorResponse('Template IDs required for extractions', 'TEMPLATE_IDS_REQUIRED', 400),
                { status: 400 }
              );
            }
          ...
          case 'datapoints':
            if (templateIds.length === 0) {
              return NextResponse.json(
                createErrorResponse('Template IDs required for data points', 'TEMPLATE_IDS_REQUIRED', 400),
                { status: 400 }
              );
            }
          ...
          default:
            return NextResponse.json(
              createErrorResponse('Invalid process type', 'INVALID_PROCESS_TYPE', 400),
              { status: 400 }
            );
        }

        debugLog('api', `AI processing completed for file ${fileIdInt}`, { processType });
