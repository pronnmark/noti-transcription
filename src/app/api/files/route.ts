...
import { withMiddleware, createApiResponse, createErrorResponse, createPaginatedResponse } from '@/lib/middleware';
    ...
    try {
      ...
      const enrichedFiles = await Promise.all(
        files.map(async (file) => {
          const transcriptionJob = await transcriptionRepo.findLatestByFileId(file.id);
          const summarizationCount = await summarizationRepo.countByFileId(file.id);
          ...
          return {
            ...
            diarizationStatus: transcriptionJob?.diarizationStatus || 'not_attempted',
            summarizationCount,
...
export const DELETE = withMiddleware(
  async (request: NextRequest, context) => {
    try {
      ...
      const filePath = join(process.cwd(), 'data', 'audio_files', file.fileName);
