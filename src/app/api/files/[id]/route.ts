...
export async function GET(
  ...
  const authenticatedHandler = handler(
    async (request: NextRequest, context) => {
      ...
      apiDebug('Fetching file details', {
        fileId,
      ...
      try {
        ...
        const transcriptionJob =
          await transcriptionRepo.findLatestByFileId(fileId);

        const transcriptionStatus = transcriptionJob
      ...
      try {
        ...
        try {
          ...
          try {
            await supabaseStorageService.deleteFiles({
              bucket: 'transcripts',
              paths: [`${fileId}.json`, `${fileId}_metadata.json`],
            });

            apiDebug('Deleted transcript files from Supabase Storage', {
              fileId,
            });
          } catch (transcriptError) {
            // Transcript files might not exist, which is fine
            apiDebug(
              'No transcript files to delete (expected)',
              transcriptError,
            );
          }
        } catch (storageError) {
          apiDebug(
            'Error deleting files from Supabase Storage (continuing with DB deletion)',
            storageError,
          );
        ...
        try {
          ...
          const transcriptionJobs =
            await transcriptionRepo.findByFileId(fileId);
