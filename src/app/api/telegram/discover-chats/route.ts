...
export async function GET(request: NextRequest) {
  try {
    ...
    const updatesResponse = await fetch(`https://api.telegram.org/bot${botToken}/getUpdates?limit=100&offset=-100`);
        ...
        if (update.message && update.message.chat) {
          ...
          if (!seenChats.has(chatKey)) {
            ...
            const chatInfo = {
              ...
              mapping_suggestion: chat.username ? `'${chat.username.toLowerCase()}': '${chat.id}',` : null,
              last_message_date: update.message.date,
              last_message_text: update.message.text?.substring(0, 50) + (update.message.text?.length > 50 ? '...' : ''),
            };
    ...
    const mappingCode = discoveredChats
      ...
      .map(chat => `  '${chat.username.toLowerCase()}': '${chat.id}', // ${chat.title}`)
      .join('\n');

    return NextResponse.json({
      discovered_chats: discoveredChats,
      total_found: discoveredChats.length,
      mapping_code: mappingCode ? `// Add these to knownUsers mapping:\n${mappingCode}` : 'No usernames found to map',
      instructions: {
        note: 'Users need to send /start to @devdashbotBot to appear here',
        add_user: 'Ask ddoskar to send /start to @devdashbotBot, then refresh this endpoint',
        update_code: 'Copy the mapping_code and add it to src/app/api/mcp/tools/route.ts knownUsers object'
      }
    });
