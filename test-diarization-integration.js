...
async function testDiarizationIntegration() {
  ...
  try {
    ...
    log(`     Successful diarizations: ${stats.successful_diarizations}`, 'green');
    ...
    if (successFile) {
      try {
        const response = await fetch(`http://localhost:5173/api/transcript/${successFile.file_id}`);
        const data = await response.json();
        
        if (data.hasSpeakers) {
          log(`  ✅ File ${successFile.file_id} API returns speakers correctly`, 'green');
          log(`     Segments: ${data.segments.length}`, 'blue');
          log(`     Has speakers: ${data.hasSpeakers}`, 'green');
        } else {
          log(`  ❌ File ${successFile.file_id} API doesn't return speaker info`, 'red');
        }
      } catch (error) {
    ...
    if (failedFile) {
      log(`  🔄 Testing retry for file ${failedFile.file_id}: ${failedFile.original_file_name}`, 'blue');
      
      try {
        const response = await fetch(`http://localhost:5173/api/transcribe/retry/${failedFile.file_id}`, {
          method: 'POST'
        });
    ...
    for (const job of recentJobs) {
      ...
      if (fs.existsSync(metadataPath)) {
        try {
          ...
          if (metadata.status && 
            ...
            if (dbHasSpeakers !== metaHasSpeakers) {
              log(`  ⚠️  File ${job.file_id}: Metadata mismatch - DB: ${dbHasSpeakers}, Meta: ${metaHasSpeakers}`, 'yellow');
            }
          } else {
            invalidMetadata++;
        ...
        log(`  ⚠️  File ${job.file_id}: Missing metadata (legacy transcription)`, 'yellow');
      }
    }
