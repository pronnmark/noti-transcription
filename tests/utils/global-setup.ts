...
async function globalSetup(config: FullConfig) {
  ...
  const testSupabaseUrl = process.env.TEST_SUPABASE_URL || process.env.SUPABASE_URL || 'http://127.0.0.1:54321';
  const testSupabaseAnon = process.env.TEST_SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY;
  const testSupabaseService = process.env.TEST_SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
  ...
  if (!testSupabaseAnon || !testSupabaseService) {
    console.error('❌ Missing Supabase keys. Please check your .env.local file.');
