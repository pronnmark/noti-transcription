...
test.describe('File Upload UI Flow', () => {
  ...
  test('should upload file through browser interface and store in Supabase', async ({
    page,
  }) => {
    ...
    const fileList = page
      .locator('[data-testid="file-list"], table, .file-item')
      .first();
    if (await fileList.isVisible()) {
      await expect(
        page
          .locator(`text=${testFile.name}`)
          .or(page.locator(`text=${fileIdText}`))
      ).toBeVisible({ timeout: 10000 });
    }

    // Track for cleanup
    const fileResponse = await page.request.get(
      `/api/files/${uploadData.fileId}`
    );
    if (fileResponse.ok()) {
  ...
  test('should handle drag and drop file upload', async ({ page }) => {
    ...
    const dropZone = page
      .locator('[data-testid="drop-zone"], .drop-zone, .upload-area')
      .first();
  ...
  test('should show upload progress and completion feedback', async ({
    page,
  }) => {
    const testFile = TestHelpers.getTestFile('test-audio-small.mp3');
    ...
    const progressIndicator = page
      .locator('[data-testid="upload-progress"], .progress, .loading, .spinner')
      .first();
  ...
  test('should display error message for invalid file types', async ({
    page,
  }) => {
  ...
  test('should redirect to file details after successful upload', async ({
    page,
  }) => {
    const testFile = TestHelpers.getTestFile('test-audio-small.mp3');
