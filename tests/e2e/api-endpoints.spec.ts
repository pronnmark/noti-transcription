...
test.describe('API Endpoints with Supabase Integration', () => {
  ...
  test.describe('POST /api/upload', () => {
    test('should upload file to Supabase and create database record', async ({
      request,
    }) => {
    ...
    test('should reject invalid file types', async ({ request }) => {
      ...
      expect(errorData.message || errorData.error).toContain(
        'Invalid file type'
      );
    });
  ...
  test.describe('GET /api/files/[id]', () => {
    test('should retrieve file metadata for Supabase-stored files', async ({
      request,
    }) => {
      // First upload a file
      const testFile = TestHelpers.getTestFile('test-audio-small.mp3');
  ...
  test.describe('GET /api/files (list files)', () => {
    test('should list files with Supabase storage paths', async ({
      request,
    }) => {
  ...
  test.describe('POST /api/transcribe-simple/[id]', () => {
    test('should download file from Supabase for transcription', async ({
      request,
    }) => {
      // Upload a file first
      const testFile = TestHelpers.getTestFile('test-audio-small.mp3');
      ...
      const transcribeResponse = await request.post(
        `/api/transcribe-simple/${uploadResponse.fileId}`
      );
  ...
  test.describe('GET /api/transcribe/status/[id]', () => {
    test('should return transcription status for uploaded files', async ({
      request,
    }) => {
      // Upload a file
      const testFile = TestHelpers.getTestFile('test-audio-small.mp3');
      ...
      const statusResponse = await request.get(
        `/api/transcribe/status/${uploadResponse.fileId}`
      );
  ...
  test.describe('File integrity verification', () => {
    test('should maintain file integrity through upload-download cycle', async ({
      request,
    }) => {
      const testFile = TestHelpers.getTestFile('test-audio-medium.wav');
